...
export const getValidAuthTokens = async (t?: string, rT?: string) => {
  const token = t || await getAuthCookie(AUTH_TOKEN);
  const refreshToken = rT || await getAuthCookie(AUTH_REFRESH_TOKEN);
